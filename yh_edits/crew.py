...
from helper_prepare_knowledge import prepare_pdf_report, download_latest_beige_book

### Add knowledge source, must add pdfs and csv in knoweldege folder
meeting_date = "20240731"
csv_file_path = "knowledge/Historical_macro_data.csv" 
excel_file_path='knowledge/fedwatch.csv'

# Dynamically generate the current macro PDF and trimmed CSV file
current_macro_pdf, trimmed_csv = prepare_pdf_report(csv_file_path, excel_file_path, meeting_date)

# Download the Beige Book PDF using the meeting date
beige_book_pdf = download_latest_beige_book(meeting_date)

# Extract just the filenames (no directories)
beige_book_pdf_name = Path(beige_book_pdf).name
current_macro_pdf_name = Path(current_macro_pdf).name
trimmed_csv_name = Path(trimmed_csv).name

pdf_source = PDFKnowledgeSource(
    file_paths=[
        beige_book_pdf_name,
        current_macro_pdf_name,
        #f"{date} dot plot description.pdf",
        "Fed Explanation.pdf",
    ]
)

csv_source = CSVKnowledgeSource(file_paths=[trimmed_csv_name])
